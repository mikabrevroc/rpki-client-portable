...
/*
 * Parse and validate a RASA-SET (RPKI AS-SET Membership).
 * Returns the rasa_set on success, NULL on failure.
 */
static struct rasa_set *
proc_parser_rasa_set(char *file, const unsigned char *der, size_t len,
    const struct entity *entp, X509_STORE_CTX *ctx)
{
	...
	if ((rasa_set = rasa_set_parse(&cert, file, entp->talid, der, len)) == NULL)
		goto out;
	...
	rasa_set->talid = a->cert->talid;

	rasa_set->expires = x509_find_expires(cert->notafter, a, &crls);
	cert_free(cert);
    ...
    BN_CTX *bn_ctx)
{
	...
	while ((entp = TAILQ_FIRST(q)) != NULL) {
		...
		case RTYPE_RASA_SET:
			...
			io_str_buffer(b, file);
			if (experimental) {
				rasa_set = proc_parser_rasa_set(file, f, flen, entp, ctx);
				if (rasa_set != NULL)
					mtime = rasa_set->signtime;
			} else {
				if (verbose > 0)
					warnx("%s: skipped", file);
				rasa_set = NULL;
