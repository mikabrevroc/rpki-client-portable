...
outputheader_json(struct validation_data *vd, struct stats *st)
{
	...
	json_do_int("rasa_sets", st->repo_tal_stats.rasa_sets);
	json_do_int("failedrasa_sets", st->repo_tal_stats.rasa_sets_fail);
	json_do_int("invalidrasa_sets", st->repo_tal_stats.rasa_sets_invalid);
...
static void
print_vrp_rasa(struct vrp_rasa *vr)
{
	size_t i;

	json_do_object("rasa", 1);
	if (vr->is_asid)
		json_do_int("authorized_as", vr->asid);
	else
		json_do_string("authorized_set", vr->asset);

	json_do_array("authorized_in");
	for (i = 0; i < vr->num_entries; i++) {
		json_do_object("entry", 1);
		json_do_string("asset", vr->entries[i].asset);
		json_do_int("propagation", vr->entries[i].propagation);
		json_do_end();
	}
	json_do_end();

	json_do_int("expires", vr->expires);
	json_do_string("ta", taldescs[vr->talid]);
	json_do_end();
}

static void
print_vrp_rasa_set(struct vrp_rasa_set *vr)
{
	size_t i;

	json_do_object("rasa_set", 1);
	json_do_string("as_set_name", vr->as_set_name);
	json_do_int("containing_as", vr->containing_as);

	json_do_array("members");
	for (i = 0; i < vr->num_members; i++) {
		json_do_object("member", 1);
		json_do_int("asid", vr->members[i].asid);
		json_do_int("propagation", vr->members[i].propagation);
		json_do_end();
	}
	json_do_end();
...
static void
output_rasa(struct vrp_rasa_tree *vrasas)
{
	struct vrp_rasa	*vr;

	json_do_array("rasas");
	RB_FOREACH(vr, vrp_rasa_tree, vrasas)
		print_vrp_rasa(vr);
	json_do_end();
}

static void
output_rasa_set(struct vrp_rasa_set_tree *vrasa_sets)
{
	struct vrp_rasa_set	*vr;

	json_do_array("rasa_sets");
	RB_FOREACH(vr, vrp_rasa_set_tree, vrasa_sets)
		print_vrp_rasa_set(vr);
	json_do_end();
}
...
output_json(FILE *out, struct validation_data *vd, struct stats *st)
{
	...
	if (experimental)
		output_rasa(&vd->vrasas);

	if (experimental)
		output_rasa_set(&vd->vrasa_sets);
